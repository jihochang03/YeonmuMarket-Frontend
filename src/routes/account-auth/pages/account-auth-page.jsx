...
const AccountAuthPage = () => {
  ...
  const handleAccountConfirm = async () => {
    try {
      const response = await axios.post('/api/account-auth', {
        name,
  ...
  if (isAccountConfirmation) {
    return (
      <div className='max-w-lg h-main-height p-1 rounded-md mx-5 flex flex-col justify-center'>
        ...
        {error && <ErrorModal message={error} onClose={() => setError('')} onRetry={handleRetry}/>}
      <button
  ...
  return (
    <div className="max-w-lg h-main-height p-1 rounded-md mx-5">
      <h3 className='w-full flex justify-center mt-2 font-semibold'>안전한 거래를 위해</h3>
      <h3 className='w-full flex justify-center mb-4 font-semibold'>몇 가지 정보가 필요해요.</h3>
      <div className='w-full bg-gray-100 flex flex-col px-4 rounded-sm pb-2 mb-4'>
        ...
        <input type="text" placeholder="" className="border p-2 mb-4 rounded-md" value={name} onChange={(e) => setName(e.target.value)} />
        ...
        <label className="block mb-2 font-bold mt-4">본인 명의의 계좌번호</label>
        <input type="text" placeholder="" className="border p-2 mb-4 rounded-md" value={accountNum} onChange={(e) => setAccountNum(e.target.value)}/>
        <label className='block mb-2 font-bold'>은행 선택</label>
        <button onClick={() => setIsBankSelection(true)} className='border p-2 rounded-md mb-2 py-8'>
          {selectedBank ? (
            <div className='flex flex-col items-center'>
              <img src={selectedBank.logo} alt={`${selectedBank.name} 로고`} className='w-12 h-12' />
        ...
        {error && <ErrorModal message={error} onClose={() => setError('')} onRetry={handleRetry}/>}
      <button className='w-full items-center bg-purple-200 flex justify-center py-3 mb-3 font-bold rounded-sm' onClick={handleSubmit}>입력 완료</button>
    </div>
  );
};
